...
describe('Product e2e test', () => {
  ...
  it('should load create Product page', async () => {
    ...
    expect(await productUpdatePage.getPageTitle().getAttribute('id')).to.match(/pluscapeApp.product.home.createOrEditLabel/);
  ...
  it('should create and save Products', async () => {
    async function createProduct() {
      ...
      await productUpdatePage.setCurrentPriceInput('5');
      expect(await productUpdatePage.getCurrentPriceInput()).to.eq('5');
      await productUpdatePage.setStandardPriceInput('5');
      expect(await productUpdatePage.getStandardPriceInput()).to.eq('5');
      await productUpdatePage.setLinkInput('link');
      expect(await productUpdatePage.getLinkInput()).to.match(/link/);
      await productUpdatePage.setPictureInput(absolutePath);
      // productUpdatePage.sizesSelectLastOption();
      // productUpdatePage.categoriesSelectLastOption();
  ...
  it('should delete last Product', async () => {
    ...
    expect(await productDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/pluscapeApp.product.delete.question/);
